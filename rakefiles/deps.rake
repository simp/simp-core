...
namespace :deps do
  ...
  Arguments:
    * :method  => The update method to use (Default => 'tracking'
         ...
         refs     => checks out each dep (by ref) according to in Puppetfile.build
  EOM
  ...
  desc 'Records the current dependencies into Puppetfile.build.'
  task :record do
    pwd        = Dir.pwd
    ...
    Dir.chdir top_dir
    File.open('Puppetfile.build','w'){|f| f.puts fake_lp.puppetfile }
    Dir.chdir pwd
  end
end
